// Getting time using AT+CCLK while setup and saving in RTC module...in loop we are calling date and time from rtc module
// AT+CCLK giving correct date and time with airtel but not with jio ( as tested previously)
#include <stdio.h>
#include <string.h>
#include <DHT.h>
#include <Wire.h>
...
RTC_DS1307 rtc;  
const String FIREBASE_HOST  = "YOUR FIREBASE URL/";
const String FIREBASE_SECRET  = "YOUR FIREBASE KEY";
...
void setup()
{
  ...
  // Set the RTC time using the AT command response
  setRtcTimeFromATCommand();
  dht.begin();
  delay(1000);
}
...
void loop()
{
   // Whenever you need the timestamp, get it from the RTC
...
void setRtcTimeFromATCommand() {
  // Call your AT command to get the time (replace with your actual AT command)
  ...
  if (SerialAT.available()) {
    ...
    if (index != -1) {
      String dateTimeStr = response.substring(index + 8, index + 26);
      int year = dateTimeStr.substring(0, 2).toInt() + 2000;
