...
void loop()
{
   ...
   String currentTime = getCurrentNetworkTime();
  if (!currentTime.isEmpty()) {
    DBG("Current Network Time:", currentTime);
  }
  Serial.println("Current Network Time:"+ currentTime);
  String data1 = get_temprature();
  // post_to_firebase(data1);
  
  delay(5000);
}
String getCurrentNetworkTime() {
  int   year    = 0;
  int   month   = 0;
  int   day     = 0;
  int   hour    = 0;
  ...
  for (int8_t i = 5; i; i--) {
    ...
    if (modem.getNetworkTime(&year, &month, &day, &hour, &min, &sec, &timezone)) {
      ...
      // Construct and return the time as a string
  ...
  // Return an empty string if unable to get network time after retries
  return "";
}
String get_temprature() {
